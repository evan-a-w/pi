...
- Exported `ENV_AGENT_DIR` and `APP_NAME` from the package root, alongside the existing `getAgentDir`/`CONFIG_DIR_NAME` exports, so callers (e.g. `@earendil-works/pi-server`'s account namespaces) can override a spawned session's agent config directory (credentials, settings, sessions) via environment variable without hardcoding the `PI_`-prefixed name.
